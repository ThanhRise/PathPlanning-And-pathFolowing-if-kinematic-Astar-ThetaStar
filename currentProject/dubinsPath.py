...
def dubinsLSL(alpha, beta, d):
    ...
    p_squared = 2 + d*d - (2*math.cos(alpha-beta)) + \
...
def dubinsRSR(alpha, beta, d):
    ...
    p_squared = 2 + d*d - (2*math.cos(alpha-beta)) + 2 * \
        d*(math.sin(beta)-math.sin(alpha))
    if p_squared < 0:
        # print('No RSR Path')
        p = -1
        q = -1
...
def dubinsRSL(alpha, beta, d):
    tmp0 = d - math.sin(alpha) - math.sin(beta)
    p_squared = -2 + d*d + 2 * \
        math.cos(alpha-beta) - 2*d*(math.sin(alpha) + math.sin(beta))
    if p_squared < 0:
        # print('No RSL Path')
        p = -1
        q = -1
        ...
        p = math.sqrt(p_squared)
        tmp2 = math.atan2((math.cos(alpha)+math.cos(beta)),
                          tmp0) - math.atan2(2, p)
...
def dubinsLSR(alpha, beta, d):
    ...
    if p_squared < 0:
        ...
        p = math.sqrt(p_squared)
        tmp2 = math.atan2((-1*math.cos(alpha)-math.cos(beta)),
                          tmp0) - math.atan2(-2, p)
...
def dubinsRLR(alpha, beta, d):
    tmp_rlr = (6 - d*d + 2*math.cos(alpha-beta) + 2 *
               d*(math.sin(alpha)-math.sin(beta)))/8
    if (abs(tmp_rlr) > 1):
        # print('No RLR Path')
        p = -1
        q = -1
        ...
        t = (alpha - math.atan2((math.cos(alpha)-math.cos(beta)), d -
             math.sin(alpha)+math.sin(beta)) + p/2 % (2*math.pi)) % (2*math.pi)
...
def dubinsLRL(alpha, beta, d):
    tmp_lrl = (6 - d*d + 2*math.cos(alpha-beta) + 2 *
               d*(-1*math.sin(alpha)+math.sin(beta)))/8
    if (abs(tmp_lrl) > 1):
        # print('No LRL Path')
        p = -1
        q = -1
...
def dubins_traj(param, step):
    ...
    x = 0
    i = 0
    length = (param.seg_final[0]+param.seg_final[1] +
              param.seg_final[2])*param.turn_radius
...
def dubins_segment(seg_param, seg_init, seg_type):
    ...
    if (seg_type == L_SEG):
        seg_end[0] = seg_init[0] + \
            math.sin(seg_init[2]+seg_param) - math.sin(seg_init[2])
        seg_end[1] = seg_init[1] - \
            math.cos(seg_init[2]+seg_param) + math.cos(seg_init[2])
...
def path_length(path_points):
    """
    Tính toán quãng đường trên Dubins path.
    """
    distances = np.sqrt(np.sum(np.diff(path_points, axis=0)**2, axis=1))
    return np.sum(distances)


def dubins_path_length(Waypoint1, Waypoint2, vel = 8, phi_lim = 12):
    """
    Tính toán quãng đường trên Dubins path.
    """
    Wptz = [Waypoint1, Waypoint2]
    param = calcDubinsPath(Wptz[0], Wptz[1], vel, phi_lim)
...
def main():
    ...
    while i < len(Wptz)-1:
        ...
        length = path_length(path)
        print("Path length: {0}".format(length))
        print(i)
